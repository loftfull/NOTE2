/**
 * Склеивает элементы текстового слоя PDF в строки.
 *
 * pdfjs отдаёт текст кусками, которые не совпадают со словами и строками:
 * один элемент может быть частью слова, а перенос строки приходит флагом
 * hasEOL на элементе. Наивное соединение через пробел даёт «сло во» и
 * «текст ,» — поэтому здесь есть правила про знаки препинания и скобки.
 *
 * Экспортируется ради тестов: это единственная часть разбора PDF, которую
