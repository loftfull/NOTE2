...
function Analysis({settings,workspace,setWorkspace,openEditor,capturePayload,clearCapturePayload}){
  ...
  return <div className="page">
    ...
    {mode==='import'&&<div className="analysisGrid">
      <Card><div className="row gap12"><div className="metricIcon"><Icon name="upload_file"/></div><div><strong>Загрузка файлов</strong><div className="small subtle">Текст, Office и книги разбираются прямо на устройстве. Из PDF сначала берётся текстовый слой; сканы и изображения идут в распознавание, аудио и видео — в расшифровку.</div></div></div><label className="dropzone compactDrop" style={{display:'block',marginTop:14}}><Icon name="folder_open" size={34}/><p style={{marginTop:6}}>{busy?'Обрабатываю…':'Выберите один или несколько файлов'}</p><p className="tiny subtle">Ничего не досочиняется: если коннектор недоступен, источник так и помечается, а не заполняется выдуманным текстом.</p><input ref={fileRef} className="hiddenFile" type="file" multiple onChange={e=>importFiles(e.target.files)}/></label></Card>
