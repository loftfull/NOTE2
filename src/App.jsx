...
function Dashboard({workspace,navigate,openEditor,newNote,openCapture}){
  ...
  const recent=[...notes].sort((a,b)=>(b.pinned?1:0)-(a.pinned?1:0)||b.updatedAt-a.updatedAt).slice(0,6)
  const daily=recent[0]||null
  ...
  return <div className="page todayPage yandexDensity">
    <section className="todayCompactHeader">
      ...
      <h1>{greeting}</h1>
      <p>Всё важное — заметки, файлы, задачи и источники — в одном личном пространстве.</p>
    </section>
...
function Library({workspace,setWorkspace,openEditor,newNote}){
  ...
  return <div className="page"><div className="row space" style={{marginBottom:14}}><div><h1 className="headline notesHeadline">Заметки</h1><p className="subtle small">{workspace.notes.length} заметок · хранятся локально</p></div><Button icon="add" onClick={newNote}>Новая</Button></div>
...
function Editor({editingId,workspace,setWorkspace,navigate,settings}){
  ...
  return <div className="editor"><div className="editorTop"><button className="iconBtn" onClick={()=>navigate('library')} aria-label="Назад"><Icon name="arrow_back"/></button><input className="titleInput" value={title} onChange={e=>setTitle(e.target.value)} placeholder="Название заметки"/><span className="tiny subtle">{status}</span><button className={`iconBtn ${note.pinned?'isActive':''}`} onClick={()=>updateFlag('pinned')} aria-label="Закрепить"><Icon name="push_pin"/></button><button className={`iconBtn ${note.favorite?'isActive':''}`} onClick={()=>updateFlag('favorite')} aria-label="В избранное"><Icon name="star"/></button><button className="iconBtn" onClick={()=>setAiOpen(v=>!v)} aria-label="Инструменты AI"><Icon name="auto_awesome"/></button></div>
    ...
    <div className={`editorBody ${aiOpen?'withAi':''}`}><textarea ref={textRef} className="contentInput" value={content} onChange={e=>setContent(e.target.value)} placeholder="Начните писать…  Используйте [[Название заметки]] для связей."/>{aiOpen&&<aside className="aiPanel"><div className="row space"><strong>Помощник</strong><button className="iconBtn" style={{width:36,height:36}} onClick={()=>setAiOpen(false)}><Icon name="close" size={18}/></button></div><div className="aiActions"><Button tone="tonal" icon="summarize" onClick={()=>ai('summarize')}>Сводка</Button><Button tone="tonal" icon="auto_fix_high" onClick={()=>ai('improve')}>Улучшить</Button><Button tone="tonal" icon="tag" onClick={()=>ai('keywords')}>Ключевые слова</Button><Button tone="tonal" icon="title" onClick={()=>ai('title')}>Название</Button></div><div className="aiOutput">{loading?'Анализирую…':aiOut||'Локальные функции работают сразу. Подключённый /api/ai добавляет модельный анализ.'}</div></aside>}</div>
    <div className="editorBottom"><Icon name="tag" size={19}/><input className="titleInput" style={{fontSize:14,fontWeight:500}} value={tags} onChange={e=>setTags(e.target.value)} placeholder="Теги через запятую"/><span className="tiny subtle">{countWords(content)} слов · {content.length} знаков · {backlinks.length} обратных ссылок</span></div>
  </div>
}
...
function Search({workspace,openEditor,settings}){
  ...
  return <div className="page"><h1 className="headline">Поиск</h1><p className="subtle small" style={{marginTop:5}}>Единый поиск по заметкам и всем проиндексированным фрагментам источников.</p><div className="segmented" style={{margin:'14px 0',maxWidth:330}}><button className={mode==='semantic'?'active':''} onClick={()=>setMode('semantic')}>Гибридный</button><button className={mode==='keyword'?'active':''} onClick={()=>setMode('keyword')}>По словам</button></div><Input value={q} onChange={e=>setQ(e.target.value)} placeholder="Найти мысль, заметку или фрагмент источника…"/>{q&&<div className="small subtle" style={{marginTop:10}}>{searching?'Ищу в источниках…':`${total} результатов`}</div>}
    ...
    {q&&!searching&&!total&&<div className="empty"><Icon name="search_off" size={42}/><p style={{marginTop:8}}>No matching notes or indexed sources.</p></div>}
...
function Graph({workspace,openEditor}){
  ...
  return <div className="page"><h1 className="headline">Knowledge Graph</h1><p className="subtle" style={{marginTop:5}}>This version derives relationships from your real note tags instead of drawing a fixed sample SVG.</p><div className="split" style={{marginTop:16}}><Card><h3>Topics</h3><div className="graph" style={{marginTop:12}}>{tags.map(([tag,notes])=><div className="graphNode" key={tag}><strong>#{tag}</strong><div className="tiny subtle">{notes.length} note{notes.length===1?'':'s'}</div></div>)}{!tags.length&&<div className="empty">Add tags to notes to build the graph.</div>}</div></Card><Card><h3>Connections</h3><div className="notesList">{workspace.notes.filter(n=>(n.tags||[]).length).map(n=><button key={n.id} className="chip" style={{textAlign:'left'}} onClick={()=>openEditor(n.id)}>{n.title} · {(n.tags||[]).map(t=>`#${t}`).join(' ')}</button>)}</div></Card></div></div>
}
...
function Settings({workspace,setWorkspace,settings,setSettings,setToast}){
  ...
  const saveAccountConfig=()=>{const nextEndpoint=accountEndpoint.trim()||'/api/account';const nextWorkspace=accountWorkspaceId.trim()||'default';setSettings(current=>({...current,accountEndpoint:nextEndpoint,accountWorkspaceId:nextWorkspace,accountEmail:accountEmail.trim(),accountRevision:current.accountWorkspaceId===nextWorkspace?(current.accountRevision||0):0}));if(accountToken)setApiSessionAuth(accountToken,nextEndpoint);setAccountMessage('Account sync configuration saved.');setToast('Account sync settings saved')}
  ...
  const saveSyncConfig=()=>{setSettings(current=>({...current,syncEndpoint:syncEndpoint.trim()||'/api/sync',syncWorkspaceId:syncWorkspaceId.trim()||'default',syncToken,syncRevision:current.syncWorkspaceId===(syncWorkspaceId.trim()||'default')?(current.syncRevision||0):0}));setSyncMessage('Legacy sync configuration saved on this device.');setToast('Legacy sync configuration saved')}
  ...
  return <div className="page settingsPageV49"><div className="settingsPageTitle"><h1 className="headline youHeadline">Профиль</h1><p className="subtle small">Настройки применяются сразу и сохраняются на этом устройстве.</p></div><div className="settingsGrid">
    ...
    <Card><div className="row space"><div><h3>Аккаунт и синхронизация устройств</h3><p className="small subtle" style={{marginTop:5}}>Separate revocable session per device. Android stores the session encrypted with a non-exportable Android Keystore AES key; web/PWA keeps it only for the current browser session.</p></div><span className="tag">rev {settings.accountRevision||0}</span></div>
      <div className="connectorGrid" style={{marginTop:12}}><label className="settingLabel">Account endpoint<Input value={accountEndpoint} onChange={e=>setAccountEndpoint(e.target.value)} placeholder="/api/account"/></label><label className="settingLabel">Workspace ID<Input value={accountWorkspaceId} onChange={e=>setAccountWorkspaceId(e.target.value.replace(/[^A-Za-z0-9_-]/g,''))} placeholder="default"/></label><label className="settingLabel">Email<Input type="email" value={accountEmail} onChange={e=>setAccountEmail(e.target.value)} placeholder="you@example.com"/></label>{!accountInfo&&<label className="settingLabel">Password<Input type="password" value={accountPassword} onChange={e=>setAccountPassword(e.target.value)} placeholder="12+ characters"/></label>}{!accountInfo&&<label className="settingLabel analysisWide">Registration token <span className="tiny subtle">only needed when creating an account</span><Input type="password" value={registrationToken} onChange={e=>setRegistrationToken(e.target.value)} placeholder="Gateway REGISTRATION_TOKEN"/></label>}</div>
      ...
      {!!accountSessions.length&&<div className="notesList" style={{marginTop:10}}>{accountSessions.map(session=><div className="task" key={session.id}><Icon name={session.current?'smartphone':'devices'} size={18}/><div style={{flex:1}}><div className="small"><strong>{session.deviceName}</strong>{session.current?' · current':''}</div><div className="tiny subtle">last seen {new Date(session.lastSeenAt||session.createdAt).toLocaleString()} · expires {new Date(session.expiresAt).toLocaleDateString()}</div></div><button className="iconBtn" style={{width:34,height:34}} title="Revoke session" onClick={()=>revokeSession(session.id)}><Icon name="logout" size={16}/></button></div>)}</div>}
      <p className="tiny subtle" style={{marginTop:10}}>Raw media blobs remain device-local. Remote snapshots retain revision conflict protection. Account password and session credential are never written to NoteAI backup files.</p>
    </Card>

    <Card><div className="row space"><div><h3>Старый режим синхронизации</h3><p className="small subtle" style={{marginTop:5}}>Backward-compatible v3.5 mode for private gateways. New installations should use account/device sessions above.</p></div><span className="tag">rev {settings.syncRevision||0}</span></div><div className="connectorGrid" style={{marginTop:12}}><label className="settingLabel">Sync endpoint<Input value={syncEndpoint} onChange={e=>setSyncEndpoint(e.target.value)} placeholder="/api/sync"/></label><label className="settingLabel">Workspace ID<Input value={syncWorkspaceId} onChange={e=>setSyncWorkspaceId(e.target.value.replace(/[^A-Za-z0-9_-]/g,''))} placeholder="default"/></label><label className="settingLabel analysisWide">Shared token<Input type="password" value={syncToken} onChange={e=>setSyncToken(e.target.value)} placeholder="Legacy SYNC_TOKEN"/></label></div><div className="row gap8" style={{marginTop:12,flexWrap:'wrap'}}><Button tone="tonal" icon="save" onClick={saveSyncConfig}>Сохранить</Button><Button tone="tonal" icon="cloud_download" onClick={pullSync} disabled={syncBusy||!syncToken||!syncWorkspaceId}>{syncBusy?'Working…':'Pull'}</Button><Button tone="tonal" icon="cloud_upload" onClick={pushSync} disabled={syncBusy||!syncToken||!syncWorkspaceId}>{syncBusy?'Working…':'Push'}</Button></div>{syncMessage&&<div className="notice" style={{marginTop:10}}>{syncMessage}</div>}</Card>

    <Card><h3>Данные и хранилище устройства</h3><p className="small subtle" style={{marginTop:5}}>Экспорт и восстановление заметок, задач, настроек, источников и индекса. Пароли и исходные медиафайлы в резервную копию не входят.</p><div className="row gap8" style={{marginTop:12,flexWrap:'wrap'}}><Button tone="tonal" icon="database" onClick={requestPersistentStorage}>Защитить локальные данные</Button>{storageState&&<span className="tag">storage {storageState}</span>}<Button tone="tonal" icon="download" onClick={exportAll}>Экспорт копии</Button><Button tone="tonal" icon="upload" onClick={()=>fileRef.current?.click()}>Импорт копии</Button><input ref={fileRef} className="hiddenFile" type="file" accept="application/json" onChange={importAll}/><Button tone="danger" icon="delete_forever" onClick={()=>{if(confirm('Delete all local notes, tasks, chats and indexed sources?')){setWorkspace({notes:[],tasks:[],chats:[]});clearSourceDb().catch(()=>{});setToast('Local workspace cleared')}}}>Удалить локальные данные</Button></div></Card>

    <Card><h3>Состояние системы</h3><div className="small statusColumns" style={{lineHeight:1.75,marginTop:8}}><div>✓ Local-first notes/tasks/settings + IndexedDB Source Vault</div><div>✓ Multimodal grounded RAG with page/time provenance</div><div>✓ Resumable large-media upload + FFmpeg segmentation</div><div>✓ Durable FIFO worker queue with restart recovery and cancellation</div><div>✓ Separate device sessions with server-side scrypt password hashing</div><div>✓ Revision-guarded account workspace sync</div><div>✓ Android Keystore AES-GCM credential bridge</div><div>✓ Web/PWA session-only credential fallback</div><div>✓ Legacy shared-token sync retained for migration</div><div>✓ Portable v3.6 backup excludes credentials/raw media</div><div className="subtle">Next gate: actual npm/Vite build, generated native android/ project, Gradle debug APK, production gateway deployment and Android 16 physical-device QA.</div></div></Card>
  </div></div>
}
